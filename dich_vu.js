...
const dich_vu = http.createServer((req, res) => {
    ...
    if (method == "GET") {
        if (url == "/dsDanhSachDatBan") {
            db.getAll("danhsachdatban").then(result => {
        ...
        if (url == "/ThemNguoidung") {
            ...
            req.on("end", () => {
                ...
                let user = JSON.parse(noi_dung_nhan);
                // Xử lý thêm vào collection Nguoi_dung.json
                db.insertOne("danhsachdatban", user).then(result => {
                    console.log(result);
                    res.writeHead(200, { "Content-Type": "text/json;charset=utf-8" });
                    res.end(JSON.stringify(kq));
                }).catch(err => {
            ...
            req.on("end", () => {
                ...
                db.deleteOne("danhsachdatban", filter).catch(result => {
                    console.log(result);
                    res.writeHead(200, { "Content-Type": "text/json;charset=utf-8" });
                    res.end(JSON.stringify(kq));
                }).catch(err => {
